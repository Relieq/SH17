...
- Output runtime cho từng biến thể: best checkpoint, metrics CSV, CSV theo từng class, prediction JSON, run manifest
...
- [ ] **Bước 2: Thêm cell install/import**
...
- [ ] **Bước 3: Chạy cell install/import**
...
def save_checkpoint(config, variant_output_dir, model, processor, optimizer, scheduler, epoch, metric_value, checkpoint_name):
    checkpoint_dir = variant_output_dir / "checkpoints"
    checkpoint_dir.mkdir(parents=True, exist_ok=True)
    checkpoint_path = checkpoint_dir / checkpoint_name
    checkpoint_path.mkdir(parents=True, exist_ok=True)
    cpu_model = model.cpu()
    cpu_model.save_pretrained(checkpoint_path)
...
for experiment_name in RUN_EXPERIMENT_NAMES:
    variant_summaries.append(train_one_experiment(experiment_lookup[experiment_name]))
...
- [ ] **Bước 7: Commit variant training runner**
